...
#include "uart.h"   // Para usar USART2->CR1 (RXNEIE)
...
void nvic_exti_pc13_button_enable(void) {
    ...
    // >>> AÑADE ESTA LÍNEA (limpia pendiente previa)
    EXTI->PR1 = (1U << 13);
...
void nvic_set_priority(uint32_t IRQn, uint8_t prio)
{
    // En STM32L4 los 4 bits efectivos de prioridad son los MSB -> prio << 4
    NVIC->IP[IRQn] = (uint8_t)(prio << 4);   // 0 = mayor prioridad, 15 = menor
}

void nvic_set_grouping(uint32_t prigroup)
{
    // Escribe VECTKEY y PRIGROUP en AIRCR
    const uint32_t VECTKEY = 0x5FAu << 16;
    SCB->AIRCR = VECTKEY | (prigroup << 8);
}
